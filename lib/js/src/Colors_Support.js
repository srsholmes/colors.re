  ...
  if (match !== 0) {
    switch (termEnv) {
      ...
      case "Hyper" : 
          return 3;
      case "iTerm.app" : 
          var match$1 = +(Caml_format.caml_int_of_string(Caml_array.caml_array_get(safeGetEnv("TERM_PROGRAM_VERSION").split("."), 0)) >= 3);
          if (match$1 !== 0) {
            return 3;
          } else {
            return 2;
...
function findMax(param) {
  return List.fold_left(Caml_obj.caml_max, 0, param);
}

var detectList_000 = isColorCompatibleCi(/* () */0);
...
var support = findMax(detectList);
